...
SceneHandling* SceneHandling::mInstance = NULL;

SceneHandling* SceneHandling::getSingletonPtr()
{
	if(mInstance == NULL)
	{
		mInstance = new SceneHandling();
	}
	return mInstance;
}
...
//ID#0002//
void SceneHandling::createDefaultScene(wxString sceneManagerName)
{
	/*
	// Create default grid
...
void SceneHandling::deleteMesh(std::string meshName, std::string sceneManagerName)
{
	try
	{
		...
		{
			SceneHandling::getSingletonPtr()->removeObjectFromSelection(meshName);
...
//ID#0002//
void SceneHandling::updateObjectTitles()
{
	for(ObjectTitleVector::iterator iter = mObjectTitlesVector.begin(); iter != mObjectTitlesVector.end(); iter++)
	{
		(*iter)->update();
...
//ID#0001//outdated
void SceneHandling::ImportPrototypeFromXML(wxString URLofXML)
{
...
//ID#0001//outdated
void SceneHandling::AttachNewScene(wxString UniqueNameOfPrototype)
{
...
//ID#0002//
void SceneHandling::release()
{
	delete SceneHandling::getSingletonPtr();
}

//ID#0001//outdated
wxArrayString SceneHandling::GetAvailablePrototypesOfDotSceneManager()
{
