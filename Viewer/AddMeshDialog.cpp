	...
	mRenderWin( NULL )
{
	...
	mRenderWinWX->Bind( wxEVT_SIZE,
		&OViSEAddMeshDialog::OnRenderWinResize,
		this );
}
...
void OViSEAddMeshDialog::updateMeshList()
{
	std::vector<std::string> groups =
	        SceneHandling::getAvailableResourceGroupNames();
	    ...
	    it++)
	{
		try
		{
			std::vector<std::string> meshlist =
			        SceneHandling::getAvailableMeshes( *it );
			    ...
			    iter++)
			{
				if(( *iter ).rfind( ".mesh" ) ==
				   ( *iter ).length() - 5)
					mMeshList->Append( wxString( iter->
							c_str(), wxConvUTF8 ));
			}
		}
...
void OViSEAddMeshDialog::OnCloseDialog( wxCloseEvent& event )
{
	...
	delete mListener;
	Ogre::Root::getSingletonPtr()->detachRenderTarget(
		"AddMeshDialogRender" );
...
Ogre::String OViSEAddMeshDialog::GetOgreHandle()
{
	...
	handle =
	        Ogre::StringConverter::toString((size_t)((HWND)mRenderWinWX->
			                                 GetHandle()));
#elif defined( __WXGTK__ )
	...
	// Put all together
	handleStream << displayStr << ':' << DefaultScreen( display ) << ':' <<
	wid;
