...
void OgreWindow::OnCreate(wxWindowCreateEvent& Event)
{
	try
	{
		...
		std::cout << (mContextCreated ? "Context created" : "Context NOT created") << std::endl;
		...
		{
			std::cout << "Unable to select RenderSystem" << std::endl; 
		}
		//	return;
		...
		this->mSceneMgr = mRoot->createSceneManager(Ogre::ST_GENERIC, "SceneManager");
