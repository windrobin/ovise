...
void SyncAttribs( T* BasicEntityView )
{
	...
	const Entity::AttributeType& Table=BasicEntityView->GetDataEntity()->GetAttributes();
	typedef Entity::AttributeType::const_iterator iterator;

	// Sync all attributes
	for (iterator i=Table.begin(); i!=Table.end(); ++i )
	{
		BasicEntityView->OnEntityAttributeChanged(BasicEntityView->GetDataEntity(),i->first,&i->second);
	}
}
