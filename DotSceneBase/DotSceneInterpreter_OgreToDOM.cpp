...
wxString DotSceneInterpreter_OgreToDOM::GetVersionString() {
	return VersionString;
}
DotSceneInterpreter_OgreToDOM::~DotSceneInterpreter_OgreToDOM( void ) { }
bool DotSceneInterpreter_OgreToDOM::Interpretation_OgreScene(
        wxString PrototypeNativeName,
        DotSceneBaseConfiguration
        *        Configuration )
{
	wxString LogMsg;

	if (PrototypeNativeName.IsSameAs( ToWxString( "" )))
		PrototypeNativeName = ToWxString( "NoName" );
	...
	this->Implementation = DOMImplementationRegistry::getDOMImplementation(
		ToXMLString( "Core" ));                                                              // Comment by HR: Core??? Has anyone a list of more valid strings? An enum would be better!

	// Create DOMDocument
	xercesc::DOMDocument* TempDOMDocument =
	        DotSceneInterpreter_OgreToDOM::Implementation->createDocument(
			0,
			ToXMLString( "scene" ),
			0 );                                                                                                               // Don't use any more, but remember: "(XMLString::transcode(this->mDestinationURI.c_str()), XMLString::transcode("scene"), 0)"

	// Create new Prototype
	this->Prototype = new ScenePrototype( PrototypeNativeName,
		TempDOMDocument );

	// Get "scene"-element and add attribute "formatVersion"...
	DOMElement* SceneElement =
	        this->Prototype->GetDOMRepresentation()->getDocumentElement();
	SceneElement->setAttribute( ToXMLString( "formatVersion" ),
		ToXMLString( this->GetVersionString()));
...
void DotSceneInterpreter_OgreToDOM::Interpretation_Nodes(
        xercesc::DOMElement* SceneElement )
{
	// Create "nodes"-element and append to "scene"-element...
	DOMElement* NodesElement =
	...
	if (!this->Selection.IsEmpty())
	{
		for (unsigned long IT = 0; IT < this->Selection.Count(); IT++)
		{
			...
			switch(OgreMediator::GetSingletonPtr()->GetObjectAccess()
			       ->GetMovableType( qMO ))
			{
				...
				if ( MO != 0 )
				{
					Ogre::SceneNode* SN =
					        static_cast<Ogre::SceneNode*>(
					                MO->
					                getParentSceneNode());
					WhiteList_STAGE1[ToWxString( SN->
								 getName())] =
					        SN;
				}
			}
	...
	else
	{
		...
		WhiteList_STAGE1[ToWxString( this->SceneMgr->getRootSceneNode()
					 ->getName())] =
		        this->SceneMgr->getRootSceneNode();
	}
#endif
	...
	{
		for(HashMap_OgreSceneNodePointer::iterator iter =
		            WhiteList_STAGE1.begin();
		    iter != WhiteList_STAGE1.end();
		    iter++)
		{
			this->RecursiveNodeTreeWalkthrough( iter->second,
				WhiteList_STAGE1,
				WhiteList_STAGE2,
				BlackList_STAGE2 );
		}
	...
	{
		for(HashMap_DOMPointer::iterator iter = WhiteList_STAGE2.begin();
		    iter != WhiteList_STAGE2.end();
		    iter++)
		{
...
void DotSceneInterpreter_OgreToDOM::RecursiveNodeTreeWalkthrough(
        Ogre::Node* SomeNode,
        HashMap_OgreSceneNodePointer
        &           WhiteList_STAGE1,
        HashMap_DOMPointer
        &           WhiteList_STAGE2,
        HashMap_DOMPointer
        &           BlackList_STAGE2 )
{
	...
	while(ChildIter.hasMoreElements())
	{
		Ogre::SceneNode* ChildSN =
		        static_cast<Ogre::SceneNode*>( ChildIter.getNext());

		if (this->Configuration->doExportNotSelectedChildToo)
		{
			RecursiveNodeTreeWalkthrough( ChildSN,
				WhiteList_STAGE1,
				WhiteList_STAGE2,
				BlackList_STAGE2 );
		}
		else
		{
			...
			if (BlackList_STAGE2.count( uChildSN ) == 1)
			{
				...
				DOMNodeList* tempDOMChildNodes =
				        Evaluate_NodeElement->
				        getElementsByTagName(
						ToXMLString( "node" ));
				if (tempDOMChildNodes->getLength() != 0)
				{
					for(XMLSize_t NodeIterator = 0;
					    NodeIterator <
					    tempDOMChildNodes->getLength();
					    ++NodeIterator)
					{
						DOMNode* CurrentNode =
						        tempDOMChildNodes->item(
								NodeIterator );
						    ...
						    == DOMNode::ELEMENT_NODE)
						{
									    ...
									    "name" )))
							{
								...
								if (uChildSN.
								    IsSameAs(
									    tempDOMChildNodeName ))           // Check if DOMElement alreay exists...
								{
									Matching_DOMElement
									        =
									                true;
				...
				{
					DOMElement* tempChild =
					        BlackList_STAGE2[uChildSN];
					Evaluate_NodeElement->appendChild(
						tempChild );
				}
			}
		}
