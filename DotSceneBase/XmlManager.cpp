...
XmlManager::XmlManager( DotSceneBaseConfiguration* Configuration ) :
	mInitialized( true )
{
...
bool XmlManager::InitXML()
{
	...
	try
	{
		...
		ParsingMsg << ToWxString(
			"XML Manager: *** XERCES initialized. ***" );
	...
	catch (const XMLException& e)
	{
		ParsingMsg.Clear();
		ParsingMsg << ToWxString(
			"XML Manager: *** Error, while initializing XERCES! Exception message is: " )
		           << ToWxString( e.getMessage());
		Logging::GetSingletonPtr()->WriteToOgreLog( ParsingMsg,
			Logging::Critical );
		return false;
	}
...
bool XmlManager::TerminateXML()
{
	...
	catch( xercesc::XMLException& e )
	{
		ParsingMsg.Clear();
		ParsingMsg << ToWxString(
			"XML Manager: *** Error, while terminating XERCES! Exception message is: " )
		           << ToWxString( e.getMessage());
		Logging::GetSingletonPtr()->WriteToOgreLog( ParsingMsg,
			Logging::Critical );
		return false;
...
bool XmlManager::SetURLofXSD( wxString URLofXSD )
{
	...
	if (this->mURLofXSD.IsRelative()) this->mURLofXSD.MakeAbsolute(
			wxFileName::GetCwd());
	...
	try
	{
		if (!wxFileName::FileExists( this->mURLofXSD.GetFullPath()))
		{
			LogMessage.Clear();
			LogMessage <<
			ToWxString( "XML Manager: Given *.xsd \"" ) <<
			this->mURLofXSD.GetFullPath() << ToWxString(
				"\" does not exist!" );
	...
	{
		Logging::GetSingletonPtr()->WriteToOgreLog( LogMessage,
			Logging::Critical );
...
bool XmlManager::SetURLofExportPath( wxString URLofExportPath )
{
	...
	if (this->mURLofExportPath.IsRelative()) this->mURLofExportPath.
		MakeAbsolute( wxFileName::GetCwd());
	...
	try
	{
		...
		{
			LogMessage.Clear();
			LogMessage << ToWxString(
				"XML Manager: Default export-path \"" ) <<
			this->mURLofExportPath.GetPath() << ToWxString(
				"\" does not exist!" );
			throw new OViSEException( ToCString( LogMessage ));
		}
...
wxString XmlManager::GetURLofXSD() {
	return this->mURLofXSD.GetFullPath();
}
wxString XmlManager::GetURLofExportPath() {
	return this->mURLofExportPath.GetPath();
}
DotSceneBaseConfiguration* XmlManager::GetConfiguration() {
	return this->mConfiguration;
}
bool XmlManager::ExportScenePrototype( ScenePrototype* Prototype,
                                       wxFileName      DestinationURL )
{
	...
	this->mImplementation = DOMImplementationRegistry::getDOMImplementation(
		XMLString::transcode( "Core" ));
	DOMLSSerializer* tempLSServializer =
	        this->mImplementation->createLSSerializer();
	tempLSServializer->writeToURI(
		Prototype->GetDOMRepresentation(),
		ToXMLString( Destination.GetFullPath()));
	...
	if (this->GetConfiguration()->doExportMeshFiles)
	{
		...
		if (!Prototype->Data.MeshDatastructures.IsEmpty())
		{
			for (unsigned int iter = 0;
			     iter < Prototype->Data.MeshDatastructures.GetCount();
			     iter++)
			{
				// First: Get pointer to mesh in memory!
				Ogre::MeshPtr tempMeshPtr =
				        Prototype->Data.MeshDatastructures[iter
				        ];
				...
				tempModifiedFileName.SetFullName( ToWxString(
						tempMeshPtr.get()->getName()));

				// Use the modified full path to export *.mesh there.
				tempSerializer.exportMesh(
					tempMeshPtr.getPointer(),
					ToOgreString( tempModifiedFileName.
						GetFullPath()));
			}
		}
		// Copy meshfiles
		...
		{
			for(unsigned int iter = 0;
			    iter < Prototype->Data.MeshFiles.GetCount();
			    iter++)
			{
				// First: Define location of source file...
				wxFileName Source(
				        Prototype->Data.MeshFiles[iter] );
				Source.MakeAbsolute(
					Prototype->Data.ResourceBaseDir );
		...
		{
			for(unsigned int iter = 0;
			    iter < Prototype->Data.MaterialFiles.GetCount();
			    iter++)
			{
				// First: Define location of source file...
				wxFileName Source(
				        Prototype->Data.MaterialFiles[iter] );
				Source.MakeAbsolute(
					Prototype->Data.ResourceBaseDir );
...
ScenePrototype* XmlManager::ImportScenePrototype( wxFileName URLofXML )
{
	...
	if (TempURLofXML.IsRelative()) TempURLofXML.MakeAbsolute(
			wxFileName::GetCwd());
	...
	this->mParser->setExternalNoNamespaceSchemaLocation( ToXMLString( this
			->mURLofXSD.GetFullPath()));
	...
	wxString ParsingMsg;
	ParsingMsg.Clear();
	ParsingMsg << ToWxString( "XML Manager: Parsing file \"" ) <<
	TempURLofXML.GetFullPath() + ToWxString( "\"... " );
	...
	catch (const XMLException& e)
	{
		...
		ParsingMsg.Clear();
		ParsingMsg << ToWxString(
			"XML Manager: XERCES's exception message is: " ) <<
		ToWxString(
			e.getMessage());
		Logging::GetSingletonPtr()->WriteToOgreLog( ParsingMsg,
			Logging::Critical );
	...
	catch (...)
	{
		...
		ParsingMsg.Clear();
		ParsingMsg << ToWxString(
			"XML Manager: Unexpected Exception occured, while pharsing with XERCES!" );
		Logging::GetSingletonPtr()->WriteToOgreLog( ParsingMsg,
			Logging::Critical );
	...
	if (((XmlErrorReporter*) this->mErrHandler )->HasValidationErrors())
		ErrorsOccured = true;
	...
	{
		xercesc::DOMNode* tempDOMWrapperNode =
		        this->mParser->getDocument()->cloneNode( true );
		xercesc::DOMDocument* tempDOMRepesentation =
		        static_cast<xercesc::DOMDocument*>( tempDOMWrapperNode );

		ScenePrototype* NewPrototype = new ScenePrototype(
			TempURLofXML.GetName(),
			tempDOMRepesentation );

		ScenePrototypeData PrototypeData;
		PrototypeData.ResourceBaseDir = URLofXML.GetPath();
		wxDir::GetAllFiles( PrototypeData.ResourceBaseDir,
			&( PrototypeData.Files ));
		...
		for (unsigned int iter = 0;
		     iter < PrototypeData.Files.GetCount();
		     iter++)
		{
			wxFileName AbsoluteFileName( PrototypeData.Files[iter] );

			wxFileName RelativeFileName( AbsoluteFileName );
			RelativeFileName.MakeRelativeTo(
				PrototypeData.ResourceBaseDir );
			PrototypeData.Files[iter] =
			        RelativeFileName.GetFullPath();

			if (RelativeFileName.HasExt())
			{
				if (RelativeFileName.GetExt().IsSameAs(
					    ToWxString( "material" )))
					PrototypeData.
					MaterialFiles.Add(
						RelativeFileName.GetFullPath());
				if (RelativeFileName.GetExt().IsSameAs(
					    ToWxString( "mesh" )))
					PrototypeData.MeshFiles.
					Add( RelativeFileName.GetFullPath());
				if (RelativeFileName.GetExt().IsSameAs(
					    ToWxString( "xml" ))) PrototypeData
					.XMLFiles.Add(
						RelativeFileName.GetFullPath());
			}
		}
