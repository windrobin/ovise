...
wxString DotSceneInterpreter_DOMToOgre::GetVersionString(
        xercesc::DOMDocument* DOMRepresentationOfScene )
{
	...
	if (DOMElement_scene->hasAttribute( ToXMLString( "formatVersion" )))
		return ToWxString( DOMElement_scene->getAttribute( ToXMLString(
					"formatVersion" )));
	else return wxString(); // Return "" (empty string) when no version-string is found!
}
#if 0
bool DotSceneInterpreter_DOMToOgre::Interpretation_DOMScene(
        xercesc::DOMDocument* DOMRepresentationOfScene,
        ...
        *                     Configuration )
{
	...
	if (!qAnchorNodeName.IsValid())
	{
		...
		LogMsg.Clear();
		LogMsg << ToWxString(
			"OViSE DOM Interpretation (abstract): Parameter 'qAnchorNodeName' is INVALID. Using RootSceneNode!" );
		Logging::GetSingletonPtr()->WriteToOgreLog( LogMsg,
			Logging::Normal );
	}
	else
	{
		if (!OgreMediator::GetSingletonPtr()->iSceneNode.Exist(
			    qAnchorNodeName ))
		{
			this->AnchorNode = SceneMgr->getRootSceneNode();
			...
			LogMsg.Clear();
			LogMsg << ToWxString(
				"OViSE DOM Interpretation (abstract): 'qAnchorNodeName' with value \"" )
			       << qAnchorNodeName.UniqueName() << ToWxString(
				"\" is not a qualified name of a Ogre::SceneNode. Using RootSceneNode instead!" );
			Logging::GetSingletonPtr()->WriteToOgreLog( LogMsg,
				Logging::Normal );
		}
		else
		{
			this->AnchorNode =
			        OgreMediator::GetSingletonPtr()->iSceneNode.
			        GetPtr(
					qAnchorNodeName );

			LogMsg.Clear();
			LogMsg << ToWxString(
				"OViSE DOM Interpretation (abstract): Using given paramter 'qAnchorNodeName' with value \"" )
			       << qAnchorNodeName.UniqueName() << ToWxString(
				"\"." );
			Logging::GetSingletonPtr()->WriteToOgreLog( LogMsg,
				Logging::Normal );
	...
	// Basic setup...
	xercesc::DOMElement *DOMElement_scene =
	        DOMRepresentationOfScene->getDocumentElement();
	...
	ChildElements =
	        DOMElement_scene->getElementsByTagName( ToXMLString( "nodes" ));
	if (ChildElements->getLength() > 0) DOMElement_nodes =
		        (xercesc::DOMElement*) ChildElements->item( 0 );

	ChildElements =
	        DOMElement_scene->getElementsByTagName( ToXMLString(
				"externals" ));
	if (ChildElements->getLength() > 0) DOMElement_externals =
		        (xercesc::DOMElement*) ChildElements->item( 0 );

	ChildElements =
	        DOMElement_scene->getElementsByTagName( ToXMLString(
				"environment" ));
	if (ChildElements->getLength() > 0) DOMElement_environment =
		        (xercesc::DOMElement*) ChildElements->item( 0 );

	// STEP 1: Optional add materials, if "doAttachExternals" is true:
	if (DotSceneInterpreter_DOMToOgre::Configuration->doAttachExternals &&
	    ( DOMElement_externals != 0 ))
	{
		// Prepare AssociatedResourceGroup, when it exists:
		Ogre::ResourceGroupManager::getSingleton().unloadResourceGroup(
			ToOgreString( this->Configuration->qResourceGroup.
				UniqueName()));

		// Add path of dotScene-File to associated recourcegroup:
		Ogre::ResourceGroupManager::getSingleton().addResourceLocation(
			ToOgreString( this->Configuration->LocationOfMeshFiles.
				GetPath()),
		...
		this->Interpretation_Externals( DOMElement_externals,
			this->Configuration->qResourceGroup );

		// Again, initialise assocated resource group:
		Ogre::ResourceGroupManager::getSingleton().
		initialiseResourceGroup( ToOgreString( this->Configuration->
				qResourceGroup.UniqueName()));
	}

	// STEP 2: Optional add nodes, if "doAttachNodes" is true:
	if (DotSceneInterpreter_DOMToOgre::Configuration->doAttachNodes &&
	    ( DOMElement_nodes != 0 ))
	{
	...
	if (DotSceneInterpreter_DOMToOgre::Configuration->doAttachEnvironment
	    && ( DOMElement_environment != 0 ))
	{
	...
	LogMsg.Clear();
	LogMsg = ToWxString(
		"OViSE DOM Interpretation (abstract): Applied offset-settings to anhcor-node." );
